...
def constructBayesNet(gameState):
    ...
    edgeTuplesList = [(X_POS_VAR, GHOST_HOUSE_VAR), (X_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, GHOST_HOUSE_VAR)]
...
def fillObsCPT(bayesNet, gameState):
    ...
    Pos2Valdic = {bottomLeftPos: BOTTOM_LEFT_VAL, topLeftPos: TOP_LEFT_VAL, bottomRightPos: BOTTOM_RIGHT_VAL, topRightPos: TOP_RIGHT_VAL}
    "P2"
    housePoses = gameState.getPossibleHouses()
    for housePos in housePoses:
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos

            center = min(housePoses, key = lambda house : util.manhattanDistance(obsPos, house))
            centerVal = Pos2Valdic[center]

            obsCPT = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            obsRows = obsCPT.getAllPossibleAssignmentDicts()

            for row in obsRows:
                ghostHouseVal, foodHouseVal, color = row[GHOST_HOUSE_VAR], row[FOOD_HOUSE_VAR], row[obsVar]
                if centerVal != ghostHouseVal and centerVal != foodHouseVal:
                    prob = 1 if color == NO_OBS_VAL else 0
                elif ghostHouseVal == foodHouseVal or foodHouseVal == centerVal:
                    ...
                    prob = 0
                obsCPT.setProbability(row, prob)
            bayesNet.setCPT(obsVar, obsCPT)  
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    return max(factor.getAllPossibleAssignmentDicts(), key = lambda a : factor.getProbability(a))
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        # food_topleft, food_topright = {FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL}, {FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL}
        # prob_food_left, prob_food_right = factor.getProbability(food_topleft), factor.getProbability(food_topright)
        # p_food_left, p_food_right = 0, 0
        for assi in factor.getAllPossibleAssignmentDicts():
            if assi[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assi[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                p_food_left = factor.getProbability(assi)
            elif assi[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assi[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                p_food_right = factor.getProbability(assi)
        leftExpectedValue =  WON_GAME_REWARD * p_food_left + GHOST_COLLISION_REWARD * (1  - p_food_left)
        rightExpectedValue =  WON_GAME_REWARD * p_food_right + GHOST_COLLISION_REWARD * (1 - p_food_right)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            #??? print(explorationEvidence)
            temp_evidence = evidence.copy()
            temp_evidence.update(explorationEvidence)
            evidence_val = max(self.computeEnterValues(temp_evidence, enterEliminationOrder))
            expectedValue += prob * evidence_val
